...
| `_exp` и `_simple` варианты решают проблему доступности контекста. | `mango_ba_prompts/prompts/*` хранит контекстные и standalone-варианты prompt assets. | Если контекст недоступен, ИИ должен перейти к диалогу, а не выдумывать missing knowledge. |
...
| Placeholder-папки выглядят как обязательный шаблон. | История Mango показывала `kb/`, `docs/`, `experiments/`, `decisions/` как точки расширения. | Новый проект может скопировать больше папок, чем реально нужно в первые часы. |
