...
## SC-8. Негативный эталон границы терминов

**Вход.** Требование ограничивает изменение «входящими звонками». Версионируемый
словарь домена связывает этот термин с «исходящим обзвоном», потому что их
операционные потоки частично перекрываются, но раздел «Ограничения» не сообщает,
включён или исключён исходящий обзвон.

**Ожидаемый отказ.** `G-mach` возвращает `missing-related-term-boundary` и
указывает связанный термин. Исправленный документ проходит проверку только если
для «исходящего обзвона» явно записано `included`, `excluded` либо
`not-applicable` с основанием. Простое упоминание без статуса не проходит.

**Почему и Golden Set, и правило.** Кейс закрепляет наблюдаемый дефект как
регрессионный пример; `EP-G7` обобщает механизм на словарь домена. Жёстко
зашивать пару «входящие/исходящий обзвон» в валидатор нельзя: словарь меняется,
а машинное правило проверяет полноту границы для любых объявленных связей.
