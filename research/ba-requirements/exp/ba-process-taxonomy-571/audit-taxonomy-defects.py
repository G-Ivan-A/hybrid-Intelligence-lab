...
(projects/ba-ai-process/ba-meta-model/) и её источника — корпуса спицы
mango_ba_prompts.
...
def d6_missing_entities(spoke: Path, hub: Path) -> dict:
    theory = read(hub / "projects/ba-ai-process/ba-meta-model/10-theory.md")
...
def d8_route_without_branching(hub: Path) -> dict:
    framework = read(hub / "projects/ba-ai-process/ba-meta-model/30-decision-framework.md")
    theory = read(hub / "projects/ba-ai-process/ba-meta-model/10-theory.md")
    branch_hits = sum(
