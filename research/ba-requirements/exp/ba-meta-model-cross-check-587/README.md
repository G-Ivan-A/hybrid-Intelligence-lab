...
| [`validate-instances.py`](validate-instances.py) | исполнимы ли контракты пакета на сыром входе реальной практики, а не только на синтетическом Golden Set | три входа и три выхода проходят `C-IN` и `C-OUT` без ошибок |
| [`2026-09-15-historical-vs-new-comparison.md`](2026-09-15-historical-vs-new-comparison.md) | в чём новый результат расходится с историческим и обосновано ли расхождение | три обоснованных расхождения, одно содержательное совпадение, один маршрутный тупик и его снятие после введения контракта маршрутизации |
...
- `runs/bcreq-1059-routed/` — тот же вход 1059 с источниками уровня
  `ST-2-CORPUS`: замер того, что меняет контракт маршрутизации источников.
  Блокирующий вопрос `Q-02` снимается цитатой корпуса, `S-SETTINGS` и `S-SCOPE`
  заполняются, маршрут не останавливается.
