...
- [`agent-delivery-and-release/`](agent-delivery-and-release/00-introduction.md)
  — десятая валидация паттерна: доставка, релиз и обратная связь агента
  (стадии L10+L12). Релизная единица как связка версий, стратегии выката и
  механизмы экспозиции, ворота выпуска (включая отдельные ворота изменения
  промпта и модели), классы дрейфа и отката, замыкание контура «прод →
  регрессионный набор → ворота».
