...
| Уровень | Содержание |
| --- | --- |
| **L3 (детализация)** | Артефакты: ось E0–E4 в `standards/`; целевой шаблон `templates/ai-solution-architecture/`; раздел уровней автономии в `AI_GOVERNANCE.md` или `standards/`. Директории: переиспользовать `docs/rfc/`, `standards/`, `templates/` — **без новых пустых каталогов**. Шаблоны документов: единый frontmatter (4 поля), kebab-case, ISO-даты. |
