...
- projects: принята [ADR-017 — архитектура `ba-ai-process`: Source, Distribution и обратная связь](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/projects/ba-ai-process/decisions/2026-09-adr-017-ba-ai-process-source-distribution.md) и выполнена первая физическая миграция (issues [#595](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/595), [#599](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/599)). Source переименован в [`projects/ba-ai-process/`](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/tree/main/projects/ba-ai-process); project-scoped ADR перенесён в `decisions/`, tests и experiments изолированы, а GigaCode CLI Distribution размещён в `dist/`. Package manifest фиксирует Source revision, adapter, allowlist и SHA-256 immutable-выходов; negative gate отклоняет RRP, ADR/RFC, backlog и прочий Source rationale. Внешнее развёртывание и runtime smoke test остаются в B-179.
- projects: [Сквозная проверка мета-модели БА с эмпирической валидацией](docs/audit/2026-09-15-ba-meta-model-cross-check-audit.md) (issue #587) — независимый аудит мета-модели и выполнения issue #577 по пунктам контракта. Вердикт `conditional`: порог переработки (отсутствие более 80% критического материала) не достигнут, поэтому реинжиниринг не выполняется, формы описанных артефактов сохранены без переименований, а достройка точечная. Доказательная база — [`exp/ba-meta-model-cross-check-587/`](research/ba-requirements/exp/ba-meta-model-cross-check-587/README.md): воспроизводимость скелета измерена на трёх завершённых прогонах корпуса (три разных скелета, `M-1 = 0.0`, семь слотов из четырнадцати не встретились ни разу), прослеживаемость проекта к `research/ai-education` — два модуля из десяти, исполнимость контрактов `C-IN`/`C-OUT` — на экземплярах, собранных из сырых входов реальных задач 1059 и 975. Валидация выполнена **без** воспроизведения ранее согласованного результата: это прямой запрет постановки, и вместо сверки «получился ли тот же документ» измерено, где новый прогон расходится с историческим и обосновано ли расхождение источником.
- projects: [Таксономия уровней источников](projects/ba-ai-process/ba-meta-model/20-taxonomy.md) `ST-1-ATTACHED`, `ST-2-CORPUS`, `ST-3-PRODUCT-DOC`, `ST-4-HUMAN` с формами доказательства `EF-INLINE`/`EF-QUOTE`, объявленным порядком обращения, правилом конфликта и правилами `SR-1`…`SR-6` — вторая служебная таксономия мета-модели. Закрывает измеренный дефект: контракт `GT-5` требовал ссылку на источник, но не говорил, где агент источник ищет, и маршрут останавливался на вопросе человеку о факте, лежащем в корпусе продукта. Прогон [`runs/bcreq-1059-routed/`](research/ba-requirements/exp/ba-meta-model-cross-check-587/README.md) измеряет эффект на том же входе: блокирующий вопрос снимается дословной цитатой корпуса, `S-SCOPE` и `S-SETTINGS` заполняются, блокирующих вопросов не остаётся. Семиуровневый вектор из постановки **сужен** до четырёх уровней осознанно, три исключённых объявлены в `out_of_slice` с причиной — необъявленная ветка неотличима от забытой.
- projects: Проекция `V-CONTRACT` (чтение стороной договора) добавлена к `V-BIZ`, `V-APPROVE`, `V-DEV` **без переименования** существующих, поле `projection` переведено в обязательные контракта `C-OUT`. До этого словарь проекций был нулевой веткой: значения объявлены, но ни один артефакт их не предъявлял. `V-CONTRACT` требует формулировки `S-FR` как обязательства «предоставлять ВОЗМОЖНОСТЬ», полноты по четырём вопросам (что, кто, где, когда) и пустого `S-OPEN`.
- projects: Три отрицательных эталона Golden Set (очередь против группы, `SLA` против `AHT`, `IVR` против автоинформатора) — результат того, что правило `EP-G7` стало исполняемым: новая проверка `validate-package.py` сверяет каждую пару `confusable_with` версионированного глоссария с наличием отрицательного случая и немедленно нашла три непокрытые пары. Правило, существовавшее текстом, начало ловить пробелы в первом же прогоне.

- projects: [Пакет исполнения MVP BCREQ](projects/ba-ai-process/dist/execution-package-gigacode-cli/README.md) (issue #580) — компиляция вертикального среза `A-IN → A-CORE → A-BCREQ` для продуктового класса «контакт-центр» в форму, разворачиваемую в чистый репозиторий-спутник копированием каталога. Пакет содержит закрытые машиночитаемые словари (12 классов артефактов, 31 операция, 10 процессов и 33 навыка с отметкой принадлежности срезу, 4 продуктовых профиля, 8 смежных терминов домена с основанием смешения); контракты `C-IN`, `C-CORE`, `C-CL`, `C-OUT`, `C-RK` как JSON Schema; [граф маршрута `RG-BCREQ-v1`](projects/ba-ai-process/dist/execution-package-gigacode-cli/routes/rg-bcreq-v1.yaml) из двенадцати узлов с предикатом на каждом ребре и политикой гейтов fail-closed; одиннадцать `SKILL.md` с обязательными полями среды `name` и `description` (`SK-0`, `SK-8`) и шестью нормативными разделами Хаба; минимальный Golden Set из двух положительных случаев и одного отрицательного — кросс-валидации словаря «Входящие» против «ИО», где ожидаемым результатом является **отказ узла**, а не документ; метрики `M-1`…`M-5` и `MP-1`…`MP-6` с формулами; чек-листы гейтов, разведённые по принципу «человек не проверяет то, что проверяет машина». Машинный гейт `G-mach` поставлен как исполняемый скрипт [`validate-package.py`](projects/ba-ai-process/dist/execution-package-gigacode-cli/tools/validate-package.py) и включён в CI вместе с восемью регрессионными проверками [`test-execution-package.sh`](tools/test-execution-package.sh), каждая из которых ломает одно правило компиляции и требует отказа: валидатор, который только принимает, гейтом не является (`M-2`). Независимость прогона от Хаба проверяется машинно: ссылка на документ Хаба в артефакте времени выполнения объявлена дефектом компиляции. Состав скопированного, преобразованного и отброшенного объявлен в [манифесте компиляции](projects/ba-ai-process/dist/execution-package-gigacode-cli/package-manifest.yaml); масштабирование — задачи B-179…B-188 (Спринт 20). Номер 20 и диапазон `B-179`…`B-188`, а не 19 и `B-169`…`B-178`: при слиянии с `main` номер 19 и идентификаторы `B-169`…`B-174` оказались заняты спринтом инициализации MVP по issue [#583](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/583), и перенумерация занятого спринта порвала бы прослеживаемость. Спринт 19 инициализирует репозиторий-спутник по шаблону HTOM, Спринт 20 разворачивает в нём пакет исполнения, поэтому `B-179` зависит от `B-170`.
- analysis: [Консолидация `ai-rules/` и `ai-governance/` в единый `ai/`: options analysis](docs/analysis/2026-09-15-ai-directories-consolidation-analysis.md) (issue #579, Creative) — независимая контрольная проверка гипотезы объединения двух корневых `ai-*` каталогов. Решение: **подход А, структуру не менять**. Ключевой критерий постановки — простота наследования в `templates/htom/` — измерен по коду генома и оказался не зависящим от раскладки корня Хаба: HTOM-команда не наследует каталоги `ai-*` вообще (геном поставляет три плоских корневых контракта, его валидатор через `resolve_one_of` уже принимает три альтернативные раскладки, ни один `ai-*` каталог не входит в `required_directories`, связь с Хабом — абсолютные URL). Консолидация не упростила бы наследование, а добавила бы четвёртую принимаемую раскладку. Выгода измерена как минус одна запись корня из 26 (~4 %); цена — 658 вхождений пути в активных артефактах, из них 229 в 20 иммутабельных файлах `docs/adr/` и `docs/rfc/`, расход механизма `path_migrations` и синхронное обновление `AGENTS.md` во всех спицах, где имя `ai-rules/` стоит в general-секциях `<forbidden>` и `<artifact_homes>`, доставляемых в спицы без изменений (ADR-012). Дополнительно подход Б даёт семантический регресс: поднимает `skills/` и `commands/` из подкласса агентских контрактов (`ai-rules/skills/`, `ai-rules/commands/`) в братья `rules/` вопреки ADR-016 и утапливает границу политика ↔ правило, которую ADR-007 ввёл на уровне корня именно как защиту от схлопывания в один bucket. Индустриальный аргумент проверен и признан нейтральным: внешние среды открывают фиксированные имена (`AGENTS.md`), а не репозиторные дома артефактов. ADR-007 остаётся в силе и не становится `superseded`; зафиксированы три триггера пересмотра.
...
- ops: удалён legacy-каталог `pr-ops/` вместе с последним оставшимся файлом — deprecated-указателем `pr-ops/README.md` (issue [#585](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/585)). Указатель был оставлен миграцией `pr-ops/` → [`ops/`](ops/README.md) от 2026-09-09 (B-119, Условие 2 раздела `P.6` [RFC оси «Среда»](docs/rfc/2026-09-04-rfc-bootstrap-environment-and-structure.md)) ровно на один цикл синхронизации, и правило отката того же RFC (`P.8`) предписывало снимать его отдельным PR не ранее чем через цикл и по отдельному подтверждению. Оба условия выполнены: цикл закрыт (после миграции прошли Спринты 17 и 18 и полная актуализация бэклога в issue #583, а следующая path-миграция — дом мета-модели БА в issue #573 — уже выполнена без редирект-заглушки вообще), подтверждение дано постановкой issue #585. **Вердикт аудита ссылок — удалять:** ни одной активной ссылки на `pr-ops/` в репозитории не осталось; все оставшиеся вхождения относятся к трём классам, которые переписывать запрещено и не требуется, — исторические снимки решений (`docs/adr/`, `docs/rfc/`, `docs/analysis/`, план `docs/superpowers/plans/`), append-only летопись `CHANGELOG.md` (158 вхождений, верных для своей даты) и машинный журнал `path_migrations` в [`.hub-profile.json`](.hub-profile.json), который и есть штатный механизм разрешения старых путей после снятия заглушки. Внешние permalink-ссылки на `blob/main/pr-ops/` ломаются — это принятая RFC неустранимая цена, зафиксированная до начала миграции.

- tools: из [`tools/validate-repository-structure.sh`](tools/validate-repository-structure.sh) убрано исключение, которым `pr-ops/README.md` выводился из-под проверки «tracked legacy file»; вместо него путь `pr-ops` добавлен в `reject_path` рядом с `governance`, `website`, `experiments` и `mkdocs.yml`, то есть каталог теперь не просто отсутствует, а запрещён к воскрешению вторым SSOT. Контракты тестов инвертированы под новое состояние: `assert_deprecated_pointer_only "pr-ops"` в [`tools/test-agents-md-integration.sh`](tools/test-agents-md-integration.sh) заменён на `assert_removed_legacy_directory "pr-ops"`, из его аудита `git grep` снят pathspec-исключением ставший ненужным `pr-ops/README.md`, а [`tools/test-post-migration-validator.sh`](tools/test-post-migration-validator.sh) вместо требования сохранять заглушку теперь требует отсутствия и каталога, и самого исключения в валидаторе и проверяет `pr-ops` в сценарии отклонения legacy-путей.
...
- projects: [Скелет `A-BCREQ`](projects/ba-ai-process/dist/execution-package-gigacode-cli/templates/bcreq-skeleton.md) разводит уровни абстракции внутри слотов: `S-FR` несёт только верхнеуровневые `ФТ-NN`, `S-SCENARIO` и `S-UI` пишутся на языке заказчика как ключевые слоты согласования с обязательным сопоставлением каждого сценария на `ФТ-NN`, детализация для разработки уходит в раскрытие `V-DEV`. Состав слотов не изменён: смешение уровней — измеренный дефект корпуса (заголовок «ФТ **и** сценарии» в двух прогонах из трёх), и лечится он проекцией, а не новым слотом.

- ops: [`ops/backlog.md`](ops/backlog.md) актуализирован по решению владельца в issue [#583](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/583) — теоретическая фаза закрыта, фокус переведён на практический запуск MVP в новом репозитории `mango-ba-ai-runtime`. **Архивированы и удалены из активного бэклога:** Спринт 3 «Ремонт структуры стандартов» (цепочка B-049…B-054 исполнена; `B-050` — рассинхрон R/A/A/Report — снят стабилизацией структуры каталогов и ADR-008, `B-054` — стресс-тест связанных документов — снят правилом `EP-G7` и Golden Set); Спринт 6 «Валидация ФТ/ТЗ» (контур исполнения `mango_ba_prompts` переведён в `legacy/archived`, сущность задач перенесена в активную работу); Спринт 7 «Триггерные входы RFC/ADR» (цикл ADR-013…ADR-016 завершён); Спринт 8 «Разделение Mango на два репозитория» (стратегия Hub + Spoke утверждена, задача исполнена). Обоснование по каждой задаче остаётся в GitHub Issues/PR и в этой записи; отдельный архивный файл бэклога не создаётся по правилу [`ops/backlog-instruction.md`](ops/backlog-instruction.md). **Добавлен раздел «Статус репозиториев экосистемы»:** `mango_ba_prompts` явно объявлен `legacy/archived` (допустим только как свидетельство по ADR-014, `LG-1`…`LG-6`; активная задача, меняющая этот репозиторий, в бэклоге запрещена), `mango-ba-ai-runtime` — единственная среда исполнения MVP; там же зафиксировано правило компиляции `EP-C1` в редакции issue #583: исполняемые файлы нового репо не содержат ссылок на исследовательские документы Хаба, требующих перехода по ним за инструкцией — ссылка допустима только как `derived_from`. **Спринт 13 переопределён** в «Развёртывание контрактов бутстрапа в целевом репо»: корневой `AGENTS.md` Хаба готов (`B-110` → `DONE`), список спиц переопределён (`B-111`), `B-112` → `CANCELLED` с переносом сохранившей силу хабовой части в новую задачу `B-169`. **Спринт 14 «Единая методология БА под ИИ» переведён целиком в `Icebox`** как преждевременная оптимизация; триггер возобновления — эмпирические метрики `M-1`…`M-5` с первых прогонов BCREQ (`B-173`); исключение — исполненный `B-135` (PR #570). Спринт сохранён, а не удалён: его фактическая база (замер 368 артефактов) — вход для возобновления. **Сформирован Спринт 19 «Инициализация MVP в `mango-ba-ai-runtime`»** (`B-170` структура по шаблону HTOM, `B-171` проектный `AGENTS.md` и базовые контракты, `B-172` машиночитаемые словари таксономий MVP-среза, `B-173` `SKILL.md` и контракты маршрута BCREQ с первым сквозным прогоном, `B-174` машинная проверка `EP-C1`, плюс перенесённый `B-069`). Номер 19, а не 18: номер 18 занят спринтом сверки с AI-PDLC (issue #575), и перенумерация порвала бы прослеживаемость. `B-068` и `B-070` перенесены в отложенные с триггером `B-173`.

- ops: [`ops/backlog-instruction.md`](ops/backlog-instruction.md) 1.0 → 1.1 — легализованы статусы `CANCELLED` (задача закрыта без исполнения, потому что её основание отпало; требует явного человеческого решения и приравнивается к `DONE` при архивации спринта) и `deferred (icebox)` (отложен блок работ целиком, триггер — появление эмпирических данных), а также уже применявшиеся на практике `deferred (triggered)` и `deferred (regular)`. Правило архивации спринта расширено с «все задачи `DONE`» до «все задачи `DONE` или `CANCELLED`»: без этого спринт с отменённой задачей не архивируется никогда. Основание ввода новых статусов по принципу Anti-Inflation — решение владельца в issue #583 («Закрыть и архивировать (Done / Cancelled)»).

- tools: [`tools/validate-repository-structure.sh`](tools/validate-repository-structure.sh) и [`tools/test-post-migration-validator.sh`](tools/test-post-migration-validator.sh) синхронизированы с архивацией: закреплённые строки удалённых Спринтов 3 и 8 (`B-054`, `B-079`, `B-084`, `issues #411, #413`) заменены закреплением нового раздела «Статус репозиториев экосистемы», Спринта 19, `B-170`, `B-173` и признака `legacy/archived`; архивированные спринты добавлены в `reject_text`, чтобы удаление было необратимым без явного решения. Проверка статуса `B-053` заменена проверкой того, что задача не воскрешена в активном бэклоге, и проверкой существования `B-173` — объявленного триггера для `B-068`, `B-070` и возобновления Спринта 14.

- ops: дом мета-модели БА и таксономий процессов и операций перенесён из `research/ba-requirements/` в [`projects/ba-ai-process/`](projects/ba-ai-process/README.md) (issue #573, B-155, направление подтверждено владельцем в [комментарии к PR #572](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/572)) — три RRP-модуля ([мета-модель](projects/ba-ai-process/ba-meta-model/00-introduction.md), [таксономия процессов](projects/ba-ai-process/ba-meta-model/process-taxonomy/00-introduction.md), [таксономия операций](projects/ba-ai-process/ba-meta-model/operation-taxonomy/00-introduction.md)) описывают реализацию БА-процессов КК Манго в GigaCode, то есть являются артефактами конкретного направления, а их дом по правилу проектных артефактов — `projects/<направление>/`, а не общий `research/`. Перемещение выполнено `git mv` с сохранением истории; все ссылающиеся пути обновлены сплошным переписыванием по репозиторию, миграция объявлена в `path_migrations` профиля Хаба. Датированные снимки фактов и экспериментальные измерители сознательно остались в `research/ba-requirements/`: это доказательная база, а не артефакт направления. Валидатор структуры получил новые пути в whitelist и в `required_files`, а [`tools/validate-rrp-links.sh`](tools/validate-rrp-links.sh) — второй корень поиска `projects`, иначе проверка правила P2 молча перестала бы покрывать перенесённые модули.

- research: правила уровней декомпозиции `SB-*` в [теории таксономии процессов](projects/ba-ai-process/ba-meta-model/process-taxonomy/10-theory.md) переформулированы по ADR-016 — `SB-1`…`SB-5` говорят о **подпроцессе**, `SB-6`…`SB-9` вводят навык как форму поставки, добавлены правила формата `FM-1`…`FM-5`. Контракт `SKILL.md` мета-модели дополнен: `SK-1` допускает навык на одну операцию, `SK-6` получил **жёсткую границу объёма** — окно контекста среды 1 000 000 токенов, по исчерпании которого прогон не продолжается (заявление владельца; замер вынесен в `B-162`), добавлены `SK-9` (обязательное вычисляемое поле `interaction`) и `SK-10` (границы формы), реестр депрекации — правило `DP-5`. Контракт человеческого гейта [таксономии процессов](projects/ba-ai-process/ba-meta-model/process-taxonomy/30-decision-framework.md) дополнен правилами `HGC-5`…`HGC-7`: технический мерж или approve pull request **не является** прохождением `G-human` — это процедурный шаг, тогда как содержательный гейт требует явного вердикта ревьюера с перечнем фактически прочитанных документов, решением, основанием и подписью; отсутствие вердикта при наличии мержа — дефект, а не основание считать гейт пройденным.
