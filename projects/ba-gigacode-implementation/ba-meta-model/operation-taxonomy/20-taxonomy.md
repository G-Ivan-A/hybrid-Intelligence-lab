...
temperature: 0.3
type: research
context: [ba, taxonomy, operation, catalog, identifiers, skill-decomposition, deprecation, issue-571, issue-577]
...
| `OC-2` | Каждая операция объявляет шесть полей контракта ([`10-theory.md`](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/projects/ba-gigacode-implementation/ba-meta-model/operation-taxonomy/10-theory.md), §4). Ниже приведены пять; шестое — ярус — вынесено в отдельную колонку. |
...
У операции два имени с разными обязанностями: стабильный глобальный `ID` для
логов, схем и ссылок и читаемый `slug` для работы БА. Формат ID —
`OP-<класс>-<номер>`: `EXT` (extract), `TRN` (transform), `GEN` (generate),
`CHK` (check), `ASM` (assess). Номер не несёт порядка или семантики и после
выдачи не переиспользуется. Полные текстовые префиксы отвергнуты как шум в
схемах; один slug без namespace отвергнут как неоднозначный вне таблицы;
скрытый `type` в метаданных отвергнут, потому что теряет однозначность в
плоском логе. Пара `ID` + `slug` сохраняет краткость и читаемость.

| Правило | Формулировка |
| --- | --- |
| `OI-1` | `ID` уникален во всём каталоге и соответствует `OP-(EXT|TRN|GEN|CHK|ASM)-NN`; машинные события и связи используют ID. |
| `OI-2` | `slug` уникален во всём каталоге и остаётся читаемым каноническим именем; интерфейс для БА показывает `ID slug`. |
| `OI-3` | ID стабилен при уточнении формулировки и не переиспользуется после депрекации; изменение когнитивного действия создаёт новую операцию по `T-2`. |
...
| ID | Операция (`slug`) | Вход | Единственный выход | Условие отказа | Ярус |
| --- | --- | --- | --- | --- | --- |
| `OP-EXT-01` | `extract-entities` | связный текст предметной области | перечень сущностей с цитатой-подтверждением на каждую | текст не содержит предметных сущностей | `G6` |
| `OP-EXT-02` | `extract-actors` | связный текст или `A-CORE` | перечень акторов: роль, тип (человек, агент, Система), цитата | роль не названа во входе явно | `G6` |
| `OP-EXT-03` | `extract-statements` | связный текст | перечень утверждений о желаемом поведении с цитатами | во входе нет утверждений о поведении | `G6` |
| `OP-EXT-04` | `extract-constraints` | связный текст или внешний документ | перечень ограничений: предмет, значение, источник | ограничения не заявлены | `G6` |
| `OP-EXT-05` | `extract-metrics` | связный текст | перечень измеримых показателей: имя, значение, единица | числовых показателей нет | `G6` |
| `OP-EXT-06` | `extract-source-reference` | элемент и его источник | ссылка: документ, раздел, страница или таймкод, дословная цитата | цитата не находится в источнике | `G6` |
...
| ID | Операция (`slug`) | Вход | Единственный выход | Условие отказа | Ярус |
| --- | --- | --- | --- | --- | --- |
| `OP-TRN-01` | `normalize-text` | сырой текст, расшифровка, письмо | нормализованный текст с сохранением атрибуции реплик | вход не является текстом | `G2` |
| `OP-TRN-02` | `deduplicate-items` | перечень однородных элементов | перечень без дублей и с записью о каждом слиянии | элементы разнородны по классу | `G2` |
| `OP-TRN-03` | `split-statement` | составное утверждение | перечень утверждений, каждое из которых атомарно | утверждение уже атомарно (возвращается отказ разбиения) | `G2` |
| `OP-TRN-04` | `classify-item` | элемент и закрытый словарь значений | элемент с ровно одним значением классификации | ни одно значение словаря не подходит | `G2` |
| `OP-TRN-05` | `map-to-slot` | элемент и объявленный скелет документа | пара «элемент → раздел» | для элемента нет раздела в скелете | `G2` |
| `OP-TRN-06` | `link-elements` | два элемента и тип связи | ребро следа: источник, приёмник, тип, основание | один из концов не существует | `G2` |
| `OP-TRN-07` | `render-projection` | структурированная модель и целевая нотация | текст проекции в нотации (UML, BPMN, таблица) | модель не полна для нотации | `G2` |
...
| ID | Операция (`slug`) | Вход | Единственный выход | Условие отказа | Ярус |
| --- | --- | --- | --- | --- | --- |
| `OP-GEN-01` | `generate-statement` | элементы `A-CORE` и целевая форма утверждения | одно утверждение целевой формы со ссылкой на основание | основание отсутствует или не покрывает утверждение | `G6` |
| `OP-GEN-02` | `generate-question` | одна неоднозначность | один вопрос, отвечаемый одним фактом | неоднозначность не сформулирована | `G2` |
| `OP-GEN-03` | `generate-acceptance-criterion` | атомарное требование | один проверяемый критерий приёмки | требование не атомарно | `G6` |
| `OP-GEN-04` | `generate-rationale` | решение или вердикт и его вход | обоснование со ссылками на элементы входа | вход не содержит оснований решения | `G6` |
| `OP-GEN-05` | `generate-abstract` | документ или его раздел | краткое изложение без новых утверждений | документ пуст | `G6` |
...
| ID | Операция (`slug`) | Вход | Единственный выход | Условие отказа | Ярус |
| --- | --- | --- | --- | --- | --- |
| `OP-CHK-01` | `check-atomicity` | утверждение | вердикт «атомарно / нет» с перечнем составных частей | вход не является утверждением | `G6` |
| `OP-CHK-02` | `check-ambiguity` | утверждение | перечень неоднозначных мест с их видом (референциальная, количественная, модальная) | вход пуст | `G6` |
| `OP-CHK-03` | `check-completeness` | набор элементов и объявленная модель полноты | перечень отсутствующих обязательных элементов | модель полноты не объявлена | `G6` |
| `OP-CHK-04` | `check-consistency` | набор утверждений | перечень пар, противоречащих друг другу, с видом противоречия | в наборе менее двух утверждений | `G6` |
| `OP-CHK-05` | `check-verifiability` | требование или критерий | вердикт с указанием способа проверки или причины его отсутствия | вход не является требованием | `G6` |
| `OP-CHK-06` | `check-source-evidence` | утверждение и ссылка на источник | вердикт «цитата существует и разрешима» | ссылка отсутствует | `G6` |
| `OP-CHK-07` | `check-vocabulary` | значение и закрытый словарь | вердикт принадлежности с ближайшими значениями при отказе | словарь не объявлен | `G3` |
| `OP-CHK-08` | `check-schema` | документ или структура и схема | вердикт соответствия с перечнем нарушений полей | схема не объявлена | `G2` |
...
| ID | Операция (`slug`) | Вход | Единственный выход | Условие отказа | Ярус |
| --- | --- | --- | --- | --- | --- |
| `OP-ASM-01` | `score-item` | элемент и объявленная шкала | значение по шкале с обоснованием | шкала не объявлена | `G1` |
| `OP-ASM-02` | `rank-items` | перечень элементов с оценками | упорядоченный перечень с правилом порядка | оценки отсутствуют | `G1` |
| `OP-ASM-03` | `match-coverage` | два набора элементов и правило соответствия | матрица покрытия с непокрытыми элементами обеих сторон | правило соответствия не объявлено | `G6` |
| `OP-ASM-04` | `classify-severity` | нарушение или разрыв и шкала значимости | одно значение значимости с обоснованием | шкала не объявлена | `G1` |
| `OP-ASM-05` | `select-option` | закрытое множество вариантов и критерий | один выбранный вариант с обоснованием и отвергнутыми | множество вариантов пусто | `G5` |
...
[таксономии процессов](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/projects/ba-gigacode-implementation/ba-meta-model/process-taxonomy/20-taxonomy.md)
...
([`process-taxonomy/30-decision-framework.md`](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/projects/ba-gigacode-implementation/ba-meta-model/process-taxonomy/30-decision-framework.md), §3).
