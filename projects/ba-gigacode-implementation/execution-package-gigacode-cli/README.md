...
отброшенного объявлен в `compilation-manifest.yaml`.
...
```sh
cp -R projects/ba-gigacode-implementation/execution-package-gigacode-cli/. <clean-directory>/
...
```sh
sh projects/ba-gigacode-implementation/execution-package-gigacode-cli/tools/validate-package.sh
```

Валидатор проверяет layout, нативный путь `.gigacode/skills/`, обязательные
...
запрещено. Подробная инструкция для человека находится в соседнем модуле
`meta-model-guides/` и не дублирует нормативные правила пакета.
