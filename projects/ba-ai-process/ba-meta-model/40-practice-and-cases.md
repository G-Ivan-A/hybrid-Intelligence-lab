...
temperature: 0.3
type: research
context: [ba, meta-model, artifact-production-model, bcreq, mvp-slice, metrics, traceability, issue-563, issue-571, issue-587]
...
5. **Проекции не являются отдельными артефактами.** `V-BIZ`, `V-APPROVE`,
   `V-DEV` — три чтения одного `A-BCREQ`. Версия 0.4 добавляет к ним четвёртое,
   `V-CONTRACT` — чтение стороной договора, где `S-FR` формулируется как
   обязательство «предоставлять **ВОЗМОЖНОСТЬ**» и проверяется на полноту по
   четырём вопросам (что, кто, где, когда), а `S-OPEN` обязан быть пуст.
   Прежние три проекции не переименованы и не заменены: добавлена четвёртая
   ветка, потому что договорное чтение существовало в практике и до модели, но
   не имело объявленного контракта. Проекция объявляется полем `projection`
   контракта выхода и потому измерима; до версии 0.4 поле было необязательным,
   то есть ветка словаря не исполнялась ни разу.
...
([`operation-taxonomy/40-practice-and-cases.md`](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/projects/ba-ai-process/ba-meta-model/operation-taxonomy/40-practice-and-cases.md)).
...
| `G-mach` отказывает | узел получает одну корректирующую попытку с перечнем нарушений; повторный отказ направляется в `G-human` | `EP-R7`: одна попытка проверяет способность исправить локальный дефект, но исключает самоподдерживающийся цикл |
| Контекст чата исчерпан | новый чат получает проекцию листа прогона и разрешимые ссылки на SSOT; свободный `memory_[project].md` не создаётся | `EP-R8`: состояние восстанавливается из проверяемого лога, а не из второй редактируемой истины |
| Термин scope имеет смежные понятия в словаре | `G-mach` требует явного статуса смежных понятий в «Ограничениях» и сверяет негативный эталон | `EP-G7`: машинно проверяется полнота границы, а не смысловая догадка о конкретной паре слов |
| Ответ на вопрос лежит в корпусе продукта, а не во входе | источник берётся по объявленному уровню `ST-2-CORPUS` с дословной цитатой и отметкой времени; вопрос человеку не задаётся | вопрос о факте, извлекаемом из корпуса, — дефект маршрутизации, а не неполнота входа; замер #587 показал, что без контракта уровней маршрут останавливается там, где останавливаться не должен |
| Уровень источника не объявлен | прогон не проходит `C-IN` | `tier` обязателен: без него «приложено к задаче» и «вспомнил агент» неразличимы |
| Уровень источника лежит вне среза | обращение запрещено, уровень объявлен в `out_of_slice` с причиной | необъявленная ветка неотличима от забытой |
| `G-human` требуется в диалоговой среде | агент прерывает выполнение, печатает один пункт, ждёт ответа; ответ становится источником уровня `ST-4-HUMAN` | `GT-5`: откладывание ревью на PR — объявленный запасной путь с отметкой `gate_form: checklist`, а не умолчание |
...
| Frontmatter пакета не совпадает с версиями слоёв Source | пакет считается протухшим и подлежит перекомпиляции | `EP-C2`, `EP-C5` |

## 6. Границы практики

- Граф и срез описывают преимущественно **проектируемое** поведение. К версии
  0.4 выполнены три ручные сборки экземпляров по контрактам пакета на сырых
  входах реальных задач
  ([`research/ba-requirements/exp/ba-meta-model-cross-check-587/`](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/exp/ba-meta-model-cross-check-587/README.md)),
  но ни один прогон не выполнен агентом в среде: целевые значения метрик
  остаются определениями, а не результатами.
- Базовая линия `M-1` подтверждена независимым замером на трёх завершённых
  прогонах корпуса: три разных скелета, `M-1 = 0.0`, семь слотов из
  четырнадцати не встретились ни разу.
