...
**Story.**
Решением фаундера от 2026-07-10 проект Mango разделяется на два репозитория с
разными жизненными циклами и аудиториями: публичный `ai-ba-playbooks`
(универсальная prompt & pattern library, архетип B, без образовательного слоя —
он остаётся в `education/` Хаба) и приватный `mango-ba-prompt-library`
(операционка Mango — промпты, процессы, прогоны, evals, internal RFC/docs, без
GitHub Actions). Смешение публичной методологии и приватной операционки в одном
репозитории мешает разным ревью-процессам, разной аудитории и разной модели
доступа, поэтому решение фиксируется как ADR, а не как разовая механическая
миграция.

**Цель.**
Создать оба репозитория, зафиксировать решение через ADR, мигрировать
артефакты из `mango_ba_prompts` и настроить одностороннюю (приватный →
публичный) синхронизацию.

**Критерий закрытия.**
Спринт закрывается, когда ADR о разделении принят, план миграции составлен,
оба репозитория созданы, артефакты физически перенесены и синхронизация
протестирована и задокументирована.

| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-079** | ADR о разделении Mango на два репозитория | **P1** | - | TODO | - (planned) | Решение фаундера 2026-07-10 (issue #413) | Зафиксировать решение через ADR: публичный `ai-ba-playbooks` + приватный `mango-ba-prompt-library`, причина разделения, целевая структура каждого репозитория, модель синхронизации и связь с Хабом (`education/`, Smart Sync, обратный RFC-путь). | Structured |
| **B-080** | План миграции Mango | **P2** | B-079 | TODO | - (planned) | ADR B-079 | Составить план миграции: какие артефакты из `mango_ba_prompts` куда переезжают (публичный/приватный репозиторий), в каком порядке и как синхронизируются взаимные ссылки. | Structured |
| **B-081** | Создание публичного репозитория `ai-ba-playbooks` | **P2** | B-079, B-080 | TODO | - (planned) | ADR B-079, план миграции B-080 | Создать публичный репозиторий с базовой структурой архетипа B (`prompt-library/`, `patterns/`, `standards/`, `examples/`, `docs/`, `templates/`, опционально `app/`) и настроить GitHub Pages. | Structured |
| **B-082** | Создание приватного репозитория `mango-ba-prompt-library` | **P2** | B-079, B-080 | TODO | - (planned) | ADR B-079, план миграции B-080 | Создать приватный репозиторий со структурой операционки (`prompts/`, `kb/processes/`, `runs/`, `evals/`, `internal-rfc/`, `internal-docs/`) и настроить локальные валидаторы без GitHub Actions. | Structured |
| **B-083** | Физическая миграция артефактов из `mango_ba_prompts` | **P2** | B-081, B-082 | TODO | - (planned) | План миграции B-080 | Перенести артефакты в оба репозитория согласно плану B-080, обновить перекрёстные ссылки и прогнать локальные валидаторы каждого репозитория. | Structured |
| **B-084** | Синхронизация приватный → публичный и её тестирование | **P2** | B-083 | TODO | - (planned) | ADR B-079 | Настроить одностороннюю синхронизацию (ручной отбор на старте, автоматизация под доказанную боль), протестировать перенос изменений и задокументировать lessons learned. | Structured |
...
| Решение фаундера 2026-07-10 (issue #413) | Разделение Mango на публичный `ai-ba-playbooks` и приватный `mango-ba-prompt-library` — источник задач B-079..B-084 (Спринт 8). |
